...
namespace easyloggingpp {
    ...
    public:
        ...
        static unsigned int convertFromString(const std::string &configStr) {
            ...
            if (configStr == "to_standard_output" || configStr == "TO_STANDARD_OUTPUT")
            ...
            if (configStr == "milliseconds_width" || configStr == "MILLISECONDS_WIDTH")
                return ConfigurationType::MillisecondsWidth;
            if (configStr == "performance_tracking" || configStr == "PERFORMANCE_TRACKING")
...
//!
            ...
            public:
                ...
                replaceAll(const std::string &str, const std::string &replaceWhat, const std::string &replaceWith) {
    ...
    static std::string getDeviceName(void) {
        ...
        return ss.str();
                ...
                static std::string getEnvironmentVariable(const char *variableName, const char *defaultVal,
                                                          const char *alternativeBashCommand = NULL) {
#if _ELPP_OS_UNIX
...
#   endif // defined(_MSC_EXTENSIONS)
            ...
            tv->tv_usec = static_cast<long>(present_ % usecOffSet);
                ...
                static std::string
                getDateTime(const std::string &bufferFormat_, unsigned int type_, internal::Constants *constants_,
                            std::size_t milliSecondOffset_ = 1000) {
                    long milliSeconds = 0;
        ...
        if ((type_ & constants_->kDateTime) || (type_ & constants_->kTimeOnly)) {
            if (GetTimeFormatA(LOCALE_USER_DEFAULT, 0, 0, kTimeFormatLocal_, dateBuffer_, kDateBuffSize_) != 0) {
                ...
                static inline double getTimeDifference(const timeval &endTime_, const timeval &startTime_) {
                    return static_cast<double>((((endTime_.tv_sec - startTime_.tv_sec) * 1000000) +
                                                (endTime_.tv_usec - startTime_.tv_usec)) / 1000);
                }
            }; // class DateUtils
        } // namespace utilities
...
//!
    ...
    public:
        ...
        inline void setFromBase(Configurations *base_) {
            ...
            std::for_each(base_->list().begin(), base_->list().end(),
                          std::bind1st(std::mem_fun(&Configurations::set), this));
        }
        ...
        bool parseFromFile(const std::string &configurationFile_, Configurations *base_ = NULL) {
            ...
            __EASYLOGGINGPP_ASSERT(fileStream_.is_open(),
                                   "Unable to open configuration file [" << configurationFile_ << "] for parsing.");
...
#   endif // _ELPP_NDK
            ...
            set(Level::Debug, ConfigurationType::Format,
                "%datetime %level [%logger] [%user@%host] [%func] [%loc] %log");
            ...
            static bool parseLine(std::string &line, unsigned int &currLevel, Configurations *conf) {
                ...
                if (isConfig(line)) {
                    ...
                    if (quotesStart != std::string::npos && quotesEnd != std::string::npos) {
                        ...
                        __EASYLOGGINGPP_ASSERT((quotesStart < quotesEnd),
                                               "Configuration error - No ending quote found in [" << currConfigStr
                                                                                                  << "]");
                        __EASYLOGGINGPP_ASSERT((quotesStart + 1 != quotesEnd),
                ...
                __EASYLOGGINGPP_ASSERT(currLevel != Level::Unknown,
                                       "Unrecognized severity level [" << currLevelStr << "]");
                __EASYLOGGINGPP_ASSERT(currConfig != ConfigurationType::Unknown,
...
//!
        ...
        private:
                ...
                switch (level_) {
                    case Level::Debug:
                        internal::utilities::LogManipulator::updateFormatValue(constants_->LEVEL_FORMAT_SPECIFIER,
                                                                               constants_->LOG_DEBUG_LEVEL_VALUE,
                                                                               origFormatCopy, constants_);
                        break;
                    case Level::Info:
                        internal::utilities::LogManipulator::updateFormatValue(constants_->LEVEL_FORMAT_SPECIFIER,
                                                                               constants_->LOG_INFO_LEVEL_VALUE,
                                                                               origFormatCopy, constants_);
                        break;
                    case Level::Warning:
                        internal::utilities::LogManipulator::updateFormatValue(constants_->LEVEL_FORMAT_SPECIFIER,
                                                                               constants_->LOG_WARNING_LEVEL_VALUE,
                                                                               origFormatCopy, constants_);
                        break;
                    case Level::Error:
                        internal::utilities::LogManipulator::updateFormatValue(constants_->LEVEL_FORMAT_SPECIFIER,
                                                                               constants_->LOG_ERROR_LEVEL_VALUE,
                                                                               origFormatCopy, constants_);
                        break;
                    case Level::Fatal:
                        internal::utilities::LogManipulator::updateFormatValue(constants_->LEVEL_FORMAT_SPECIFIER,
                                                                               constants_->LOG_FATAL_LEVEL_VALUE,
                                                                               origFormatCopy, constants_);
                        break;
                    case Level::Verbose:
                        internal::utilities::LogManipulator::updateFormatValue(constants_->LEVEL_FORMAT_SPECIFIER,
                                                                               constants_->LOG_VERBOSE_LEVEL_VALUE,
                                                                               origFormatCopy, constants_);
                        break;
                    case Level::QA:
                        internal::utilities::LogManipulator::updateFormatValue(constants_->LEVEL_FORMAT_SPECIFIER,
                                                                               constants_->LOG_QA_LEVEL_VALUE,
                                                                               origFormatCopy, constants_);
                        break;
                    case Level::Trace:
                        internal::utilities::LogManipulator::updateFormatValue(constants_->LEVEL_FORMAT_SPECIFIER,
                                                                               constants_->LOG_TRACE_LEVEL_VALUE,
                                                                               origFormatCopy, constants_);
                        break;
                }
            ...
            template<typename T>
            void setValue(unsigned int level_, const T &value_, internal::ConfigurationMap<T> &map_,
                          bool skipLEVEL_ALL = false) {
...
#define _ELPP_STREAM(l) (*(l->stream()))
        ...
        public:
            Writer(const std::string &loggerId_,
                ...
                if (logger_ == NULL) {
                    __EASYLOGGINGPP_ASSERT(logger_ != NULL,
                                           "Logger [" << loggerId_ << "] not registered or configured yet!");
                    proceed_ = false;
                }
#if _ELPP_ENABLE_MUTEX
...
#   endif // (!_ELPP_QT_5)
            ...
            void buildAndWriteLine(void) {
                ...
                if ((f_ & constants_->kDateOnly) || (f_ & constants_->kTimeOnly) || (f_ & constants_->kDateTime)) {
                    v_ = internal::utilities::DateUtils::getDateTime(dateFormat,
                                                                     f_, constants_,
                                                                     conf_->millisecondsWidth(Level::All));
                ...
                if (f_ & constants_->kLogMessage) {
                    ...
                    internal::utilities::LogManipulator::updateFormatValue(fs_, logger_->stream()->str(), currLine_,
                                                                           constants_);
                }
                log();
            }
    ...
    public:
        ...
        // Link to source code
        static inline const std::string sourceCodeLink(void) {
            return std::string("https://github.com/mkhan3189/EasyLoggingPP");
        }
...
//!
    ...
    public:
        ...
        //!
        static inline void
        setDefaultConfigurations(Configurations &configurations, bool configureExistingLoggers = false) {
        ...
        public:
            ...
            filename(internal::TypedConfigurations *conf_, unsigned int level_ = Level::All) {
                __EASYLOGGINGPP_ASSERT(conf_ != NULL, "Invalid TypedConfigurations provided - nullptr");
            ...
            static inline bool
            toStandardOutput(internal::TypedConfigurations *conf_, unsigned int level_ = Level::All) {
                __EASYLOGGINGPP_ASSERT(conf_ != NULL, "Invalid TypedConfigurations provided - nullptr");
            ...
            static inline const std::string &
            logFormat(internal::TypedConfigurations *conf_, unsigned int level_ = Level::All) {
                __EASYLOGGINGPP_ASSERT(conf_ != NULL, "Invalid TypedConfigurations provided - nullptr");
            ...
            static inline int
            millisecondsWidth(internal::TypedConfigurations *conf_, unsigned int level_ = Level::All) {
                __EASYLOGGINGPP_ASSERT(conf_ != NULL, "Invalid TypedConfigurations provided - nullptr");
            ...
            static inline bool
            performanceTracking(internal::TypedConfigurations *conf_, unsigned int level_ = Level::All) {
                __EASYLOGGINGPP_ASSERT(conf_ != NULL, "Invalid TypedConfigurations provided - nullptr");
            ...
            logRollOutSize(internal::TypedConfigurations *conf_, unsigned int level_ = Level::All) {
                __EASYLOGGINGPP_ASSERT(conf_ != NULL, "Invalid TypedConfigurations provided - nullptr");
